...
//read from file 
fs.createReadStream("data/all_split/trad/trad_smoke.csv",[{flags: 'rs+'}]).pipe(converter);
fs.createReadStream("data/all_split/invest/invest_smoke.csv",[{flags: 'rs+'}]).pipe(converter2);
...
module.exports = {
  'Open Tradition' : function (browser) { converter.on("end_parsed", function (jsonArray) { for (i = 0; i < 3 ; i ++)  { 
      browser
        .frame(null)
        .useCss()
        ...
        .saveScreenshot('./data/all_split/trad/' +jsonArray[i]['number']+'newopen.png')
        ...
        */
        ...
        if(result.value == '0' ) {writeStream.write('payamount= 0'+',')} else {
          ...
          writeStream.write(money+',')
		    ...
		    .waitForElementPresent("//div[@classname='header_logo_ls']", 30000) 
        .saveScreenshot('./data/all_split/trad/' +jsonArray[i]['number']+'payamount.png')
        ...
        .saveScreenshot('./data/all_split/trad/' +jsonArray[i]['number']+'search.png')

   }})
  },


  'Open Invest' : function (browser) { converter2.on("end_parsed", function (jsonArray) { for (k = 0; k < jsonArray.length ; k ++)  { 
      browser
        .useCss()
        .url('http://210.13.77.68:10013/ls/logoutPage.do')
        .waitForElementPresent('body', 30000)
        ...
        // Open New 
        .clearValue('input[name=submissionDate_minguo]')
        .setValue('input[name=submissionDate_minguo]', jsonArray[k]['date'])
        .useXpath()
        .setValue("//input[@name='brbdAcceptanceVO.policyCode_text']",jsonArray[k]['number'])
        .getAttribute("//input[@name='brbdAcceptanceVO.policyCode_text']", "value" ,function(result){
        writeStream.write('\r\n'+result.value+',')
        })
        var id1 = makeid()
        ...
        .setValue("//input[@name='brbdAcceptanceVO.productId_text']", jsonArray[k]['code'])
        .getAttribute("//input[@name='brbdAcceptanceVO.productId_text']", "value" ,function(result){
        writeStream.write(result.value+',')
        })
        .setValue("//input[@name='brbdAcceptanceVO.registerCode']", jsonArray[k]['login'])
        .setValue("//input[@name='brbdAcceptanceVO.acptOrg_text']", '')
        .click("//input[@name='save']")
        ...
        .useXpath()
        .waitForElementPresent("//input[@classname='textfield_null text1']", 10000)
        .setValue("//input[@classname='textfield_null text1']", jsonArray[k]['number'])
        .click("//input[@name='search']")
        .waitForElementVisible("//tr[@classname='odd']", 10000)
        .pause(2000)
        .click("//tr[@classname='odd']")
        ...
        .waitForElementPresent("(//input[@name='__btnSave'])[position()=3]", 30000)
        .pause(1000) 
        .setValue("//input[@name='applyVersion']", jsonArray[k]['version']) 
        .setValue("//input[@name='applyDate_minguo']", jsonArray[k]['date']) 
        .click("//input[@name='rowid']")
        .click("//input[@name='__btnModify']")
        ...
        .setValue("//input[@name='insured.marriageId_text']", '1')
        .setValue("//input[@name='policyHolderName']", 'Kobe'+Math.floor((Math.random() * 1000000) + 1))
        .setValue("//input[@name='policyHolderBirthDay_minguo']", '860801')
        .setValue("//input[@name='policyHolderGender_text']", '1')
        .setValue("//input[@name='policyHolderJobCateId_text']", 'A101')
        .setValue("//input[@name='policyHolderJobClass']", '1')
        .setValue("//textarea[@name='policyHolderAddrAddress1']", 'tester')
        ...
        .setValue("//input[@name='coverage.amount']", jsonArray[k]['getamount'])
        .getAttribute("//input[@name='coverage.stdPremAf']", "class" ,function(result){
         if (result.value =='textfiled textfield_null right readOnly ro'){
          browser
            .setValue("(//input[@name='coverage.stdPremAf'])[position()=2]", jsonArray[k-1]['payamount'])
            console.log('payamount'+i)
          } else {
          browser
            .setValue("//input[@name='coverage.stdPremAf']", jsonArray[k-1]['payamount'])
            console.log('payamount'+k)
          }
        })
        .setValue("//input[@name='coverage.applyAmount']", jsonArray[k]['flexible'])
        ...
        .getAttribute("//input[@name='coverage.payType_text']", "class" ,function(result){
         if (result.value =='textfiled textfield_null readOnly ro'){} else {
          browser
            .setValue("//input[@name='coverage.payType_text']", jsonArray[k-1]['paytype'])
            console.log('paytype'+k)
          }
        })
        ...
        */
        ...
        for (j = 0; j < 3 ; j ++) {
        browser
        .setValue("//input[@name='investRate.fundCode']", jsonArray[k]['fundcode'+j])
        .setValue("//input[@name='investRate.assignRate']", jsonArray[k]['ratio'+j])
        ...
        .url('http://210.13.77.68:10013/ls/arap/cash/recv/counter/search.do?syskey_request_token=752ba247eba263311fb36ec58db42536&current_module_id=300168')
        .waitForElementPresent("//input[@classname='button btn']", 30000) 
        .setValue("//input[@name='policyNumber']", jsonArray[k]['number'])
        .click("//input[@classname='button btn']")
        .waitForElementPresent("(//input[@classname='button btn'])[position()=1]", 30000) 
        .getAttribute("//input[@name='totalIP']", "value" ,function(result){
        browser
        ...
        .click("//input[@name='voucherDate_minguo']")
        .pause(2000)
        .setValue("//input[@name='voucherDate_minguo']", jsonArray[k]['date'])
        .click("//select[@name='account']")
        .keys(['\uE015', '\uE006'])
        .click("(//input[@classname='button btn'])[position()=1]")
        .waitForElementPresent("//table[@id='table2']", 30000)
        ...
        .click("(//input[@classname='button btn'])[position()=4]")
        .waitForElementPresent("//div[@classname='header_logo_ls']", 30000) 
        .saveScreenshot('./data/all_split/invest/' +jsonArray[k]['number']+'payamount.png')
        ...
        .click("//input[@name='qryType']")
        .pause(1000)
        .setValue("//input[@name='policyCode_text']", jsonArray[k]['number'])
        .pause(1000)
        .click("//input[@name='qryType']")
        .pause(1000)
        .click("//input[@classname='button btn']")
        ...
        .saveScreenshot('./data/all_split/invest/' +jsonArray[k]['number']+'search.png')
